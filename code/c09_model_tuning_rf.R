...
hyperparam_tuning <- function(category_i){
  ...
  tune_model <- rand_forest(trees = tune(), 
                            min_n = tune()) %>% # Model type
  ...
  tune_grid_values <- grid_space_filling(trees(),
                                         min_n(),
                                         size = 30,
                                         type = "max_entropy")
  ...
  tune_model <- rand_forest(trees = model_hyperparams$trees, 
                            min_n = model_hyperparams$min_n) %>% # Model type
    set_engine("ranger") %>% # Model engine
    set_mode("regression") # Model mode
...
save(tuning_results, file = "data/10_model-output/model_tuning_rf.RData")
